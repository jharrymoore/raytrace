					...
					if lights.len() > 0
						...
						&& depth > (max_depth - 2) {
							for light in lights {
								...
								light_blue += albedo.green * target_color.blue
							}
				...
				Some(sky) => {
					match &sky.texture {
						...
						Some((pixels, width, height, _)) => {
							...
							let y = ((1. - t) * (*height - 1) as f32) as usize;
							let pixel_red = &pixels[(y * *width + x) * 3];
							let pixel_green = &pixels[(y * *width + x) * 3 + 1];
							let pixel_blue = &pixels[(y * *width + x) * 3 + 2];
							Srgb::new(
								0.7 * *pixel_red as f32 / 255.,
								0.7 * *pixel_green as f32 / 255.,
								0.7 * *pixel_blue as f32 / 255.,
							)
						}
					}
				}
...
fn render_line(pixels: &mut [u8], scene: &Config, lights: &Vec<Sphere>, y: usize) {
	...
	for x in 0..bounds.0 {
		let mut pixel_colors: Vec<f32> = vec![0.; 3];
		for _s in 0..scene.samples_per_pixel {
			let u = (x as f64 + rng.gen::<f64>()) / (bounds.0 as f64 - 1.);
			let v = (bounds.1 as f64 - (y as f64 + rng.gen::<f64>())) / (bounds.1 as f64 - 1.);
